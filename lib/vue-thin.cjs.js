...
function effect(handle, options = {}) {
    ...
    console.log("🚀 ~ file: effect.ts:53 ~ effect ~ runner:", runner);
    return runner;
}
/**
 * 依赖收集
...
class Ref {
    ...
    get value() {
        if (isTracking()) {
            ...
            console.log('🚀 ~ file: ref.ts:21 ~ Ref ~ getvalue ~ this.deps:', this.deps);
        }
        return this._value;
    }
    set value(newValue) {
...
console.log("🚀 ~ file: index.ts:42 ~ renderer:", renderer);
// 外部调用的createApp方法
function createApp(...args) {
    console.log("🚀 ~ file: index.ts:44 ~ createApp ~ args:", args);
